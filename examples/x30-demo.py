...
- You can also control the robot posture (stand up or lie down) using `dog_posture_control`. Note that the robot must stand up ('stand_up') before it can navigate.
- Posture changes take several seconds; the tool blocks until completion and automatically retries on failure.
...
def initialize_rosbot_tools() -> list[BaseTool]:
    ...
    return [
        ...
        NavigateToPoseBlockingTool(
            connector=connector,
            frame_id="company_part",
            action_name="multi_map_navigate_to_pose",
        ),
        GetCurrentPoseTool(
            connector=connector,
            frame_id="world",
            robot_frame_id="horizon",
        ),
        DogPostureControlTool(connector=connector),
    ]
